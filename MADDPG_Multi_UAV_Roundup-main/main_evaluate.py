...
def plot_rewards_excel(excel_path,output_dir):
    df = pd.read_excel(excel_path)

    # 绘制折线图
    plt.figure(figsize=(12, 8))
    plt.plot(df['model1'], label='MODEL 1', marker='o')
    plt.plot(df['model2'], label='MODEL 2', marker='x')
    plt.plot(df['model3'], label='MODEL 3', marker='^')
    ...
    plot_filename = os.path.join(output_dir, 'rewards_trend.png')
...
if __name__ == '__main__':
    ...
    obs = env.reset()
    rewards_excel_file ='Reward_trend.xlsx'
    output_rewards_dir='rewards_output'
    ...
    for frame in range(10000):  # Loop for each frame
        ...
        plot_rewards_excel(rewards_excel_file,output_rewards_dir)
